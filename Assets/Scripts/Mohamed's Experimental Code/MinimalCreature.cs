...
public class MinimalCreature : MonoBehaviour
{
    ...
    // Type
    public enum MinCreatureType { Albert, Kai }
    public MinCreatureType type;
    ...
    private MinimalCreature nearestOpponent = null;
    private MinimalCreature nearestTeammate = null;  // Reference to nearest teammate
    ...
    */
    ...
    private void DetectAllInRange(float range)
    {
        ...
        for (int i = 0; i < numColliders; i++)
        {
            ...
            else if ((type == MinCreatureType.Albert && collider.CompareTag("Albert")) ||
                     (type == MinCreatureType.Kai && collider.CompareTag("Kai")))
            {
                MinimalCreature teammate = collider.GetComponent<MinimalCreature>();
            ...
            else if ((type == MinCreatureType.Albert && collider.CompareTag("Kai")) ||
                     (type == MinCreatureType.Kai && collider.CompareTag("Albert")))
            {
                MinimalCreature opponent = collider.GetComponent<MinimalCreature>();
    ...
    private void DetectTeammatesInRange(float range)
    {
        ...
        string sameTypeLayer = (type == MinCreatureType.Albert) ? "Alberts" : "Kais";
        ...
        for (int i = 0; i < numSameType; i++)
        {
            ...
            MinimalCreature other = collider.GetComponent<MinimalCreature>();
    ...
    private void DetectOpponentsInRange(float range)
    {
        ...
        string oppositeTypeLayer = (type == MinCreatureType.Albert) ? "Kais" : "Alberts";
    ...
    private void DetectObjectsWithRays()
    {
        ...
        RaycastHit2D teammateHit = rayShooter.GetNearestHitByTag(type == MinCreatureType.Albert ? "Albert" : "Kai");
        RaycastHit2D opponentHit = rayShooter.GetNearestHitByTag(type == MinCreatureType.Albert ? "Kai" : "Albert");
        ...
        {
            MinimalCreature teammate = teammateHit.collider.GetComponent<MinimalCreature>();
        ...
        {
            MinimalCreature opponent = opponentHit.collider.GetComponent<MinimalCreature>();
    ...
    private void CalculateRangeIndicators()
    {
        ...
        if (nearestOpponent != null && nearestOpponentDistance <= bowRange)
        {
            ...
            for (int i = 0; i < hitCount; i++)
            {
                ...
                {
                    MinimalCreature hitCreature = hit.collider.GetComponent<MinimalCreature>();
                    if (hitCreature != null)
                    {
                        if (hitCreature.type != type && hit.collider.gameObject == nearestOpponent.gameObject)
                        {
                            // We hit the opposite type MinimalCreature before any obstacles
    ...
    public float[] GetObservations()
    {
        ...
        // 0 when outside FOV, 0 at FOV border, decreases linearly to -1 when hugging MinimalCreature (analogous to a spring force pushing away from target)
    ...
    private void FixedUpdate()
    {
        try
        {
            ...
            catch (System.Exception e)
            {
                ...
                string errorMessage = $"Error in MinimalCreature FixedUpdate action processing: {e.Message}\nStack trace: {e.StackTrace}";
                if (LogManager.Instance != null)
                {
        ...
        catch (System.Exception e)
        {
            ...
            string errorMessage = $"CRITICAL ERROR in MinimalCreature FixedUpdate: {e.Message}\nStack trace: {e.StackTrace}\nMinimalCreature info: Type={type}, Generation={generation}, Health={health}, Age={lifetime}";
            if (LogManager.Instance != null)
            {
                LogManager.LogError(errorMessage);
            }
    ...
    */
    ...
    public void TakeDamage(float damage, MinimalCreature byWhom = null)
    {
